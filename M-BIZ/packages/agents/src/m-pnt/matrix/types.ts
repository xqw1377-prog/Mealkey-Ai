...
/**
 * 三理论专用 LLM Adapter — 与 @mealkey/agent-sdk 解耦
 * evaluate 的 options.llm 传进来就可以用，不传就纯 heuristic
 */
export interface TheoryLLMAdapter {
  chat(params: {
...
export interface TheoryAgent {
  ...
  evaluate(
    ...
    options?: {
      /** LLM adapter；传入则 hybrid（LLM→heuristic），不传则纯 heuristic */
