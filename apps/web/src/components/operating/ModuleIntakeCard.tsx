  ...
  return (
    <section className="rounded-[22px] border border-[rgba(24,24,23,0.08)] bg-white p-4 shadow-[0_14px_28px_rgba(24,24,23,0.04)]">
      <div className="flex flex-col gap-1.5">
        <p className="text-[13px] leading-5 tracking-[0.01em] text-[#66735E]">最少信息收集</p>
          ...
          return (
            <label key={field.id} className="block space-y-2">
              <div className="flex items-center justify-between gap-3">
                <span className="text-[12px] leading-5 tracking-[0.01em] text-[#66735E]">{field.label}</span>
                {speechSupported && onToggleVoiceField ? (
                  <button
                    type="button"
                    onClick={() => onToggleVoiceField(field)}
                    className={`inline-flex min-h-10 items-center justify-center gap-2 rounded-full px-3 py-2 text-[12px] transition active:scale-[0.98] ${
                      active
                        ? "bg-[#181817] text-white"
                        : "border border-[rgba(24,24,23,0.08)] bg-[#F5F3EE] text-[#202124]"
                    }`}
                    aria-label={active ? `停止录入${field.label}` : `语音录入${field.label}`}
                    title={active ? "停止语音" : "语音录入"}
                  >
      ...
      {speechError ? <p className="mt-3 text-[12px] leading-5 text-[#B47C5C]">{speechError}</p> : null}
