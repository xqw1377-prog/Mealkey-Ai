...
| **L0 Agent 平台架构** | `docs/MEALKEY_AGENT_PLATFORM_ARCHITECTURE_V1.md` | **Gateway · Context API · SDK · 独立部署 · 审核 · 能力市场 · 官方/第三方共存** | **否** — 禁 Agent 直连 Prisma；禁 Core 再堆业务 Agent 大包；拆仓可渐进 |
...
13ac. **Agent Platform Architecture V1 已冻结**（2026-07-21）：独立部署；Agent Gateway；Context API；SDK；Permission；审核；Marketplace；官方/第三方共存；Core packages 变薄、业务 Agent 外置。真源 `MEALKEY_AGENT_PLATFORM_ARCHITECTURE_V1.md`。**下一刀 = 开发者接入流程（7 日上手）+ 诊断 Agent 按独立产品形状推进**。  
