/**
 * M-ED 签字交付包 — L5 就绪校验 + Markdown 存档
 */
import {
...
export type MedSignOffCheck = SeatSignOffCheck;
export type MedSignOffReadiness = SeatSignOffReadiness;

export function evaluateMedSignOffReadiness(
  project: AgentConsultingProject,
): MedSignOffReadiness {
  const withDepth = enrichConsultingWithDomainDepth("m-ed", project);
  const contract = withDepth.assets.governanceContract as
    | GovernanceContract
    | undefined;
  const pack = withDepth.assets.executionRoadmap?.governancePack;
  return evaluateSeatSignOffReadiness(withDepth, {
    ...
    contractLabel: "治理决策合同已冻结",
    packLabel: "协议清单包已生成",
  });
}

export function signMedStrategyReport(
  project: AgentConsultingProject,
  ...
  const withDepth = enrichConsultingWithDomainDepth("m-ed", project);
  const readiness = evaluateMedSignOffReadiness(withDepth);
  if (!readiness.ok) {
...
export function buildMedSignOffPackageMarkdown(
  project: AgentConsultingProject,
  opts?: { preview?: boolean },
): string {
  const contract = project.assets.governanceContract as
    | GovernanceContract
    | undefined;
  const decision = project.assets.decisionArtifact;
  const readiness = evaluateMedSignOffReadiness(project);
  ...
  const pack = project.assets.executionRoadmap?.governancePack;

  return [
    `# 股权治理决策 · 签字交付包${isPreview ? "（草稿预览）" : ""}`,
    "",
    isPreview
    ...
    contract
      ? [
          ...
          `**先锁：** ${contract.lockFirst}`,
          contract.controlFloor
            ? `**控制权底线：** ${contract.controlFloor}`
            : "",
          contract.mustSign ? `**必须成文：** ${contract.mustSign}` : "",
          "",
    ...
    "",
    pack?.markdown ||
      contract?.governancePackMarkdown ||
      contract?.wallCard ||
      "（无）",
    "",
    "---",
...
export function medSignOffPackageFilename(
  project: AgentConsultingProject,
  opts?: { preview?: boolean },
): string {
  ...
  const tag =
    opts?.preview || project.assets.signOffStatus !== "signed"
      ? "草稿"
      : "正式";
  return `股权治理决策_签字交付包_${tag}_${date}.md`;
}
