/**
 * M-MKT 签字交付包 — L5 就绪校验 + Markdown 存档
 */
import {
...
export type MmktSignOffCheck = SeatSignOffCheck;
export type MmktSignOffReadiness = SeatSignOffReadiness;
...
export function signMmktStrategyReport(
  project: AgentConsultingProject,
...
export function buildMmktSignOffPackageMarkdown(
  project: AgentConsultingProject,
  ...
  return [
    ...
    contract
      ? [
          ...
          `**进入方式：** ${contract.entryMode}`,
          contract.cityScene ? `**城市/场景：** ${contract.cityScene}` : "",
          contract.menuPilot ? `**主推：** ${contract.menuPilot}` : "",
          "",
...
export function mmktSignOffPackageFilename(
  project: AgentConsultingProject,
  opts?: { preview?: boolean },
): string {
  ...
  const tag =
    opts?.preview || project.assets.signOffStatus !== "signed"
      ? "草稿"
      : "正式";
  return `市场进入决策_签字交付包_${tag}_${date}.md`;
}
